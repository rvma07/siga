...
;(function($) {
	...
	$.fn.yiiGridView = function(options) {
		return this.each(function(){
			...
			if(settings.selectableRows > 0) {
				$('#'+id+' .'+settings.tableClass+' > tbody > tr').live('click',function(){
					if(settings.selectableRows == 1)
						$(this).siblings().removeClass('selected ui-state-highlight');
					$(this).toggleClass('selected').toggleClass('ui-state-highlight');
					if(settings.selectionChanged != undefined)
						settings.selectionChanged(id);
				});
			}

			$('#'+id+' .'+settings.tableClass+' > tbody > tr').live('mouseover',
				function(){
			...
			$('#'+id+' .'+settings.tableClass+' > tbody > tr').live('mouseout',
				function() {
